...
const NASA_TAP_URL =
  'https://exoplanetarchive.ipac.caltech.edu/TAP/sync?query=select+pl_name,hostname,pl_discmethod,pl_orbper,pl_orbsmax,pl_masse,pl_rade,st_lum,st_mass,st_teff,sy_dist+from+pscomppars+where+default_flag=1&format=json';
...
const ingest = async () => {
  ...
  let rawData;
  try {
    const response = await axios.get(NASA_TAP_URL, { timeout: 60000 });
    rawData = response.data;
    console.log(`📡 Fetched ${rawData.length} records from NASA TAP endpoint.`);
  } catch (err) {
  ...
  try {
    for (const row of rawData) {
      ...
      const planet = {
        ...
        hostname,
        discovery_method: row.pl_discmethod || null,
